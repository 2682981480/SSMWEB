            ...
            <div class="col-sm-12">
                <div class="ibox float-e-margins">
                    <div class="ibox-title">
                        <h5>基本 ,分类，查找，<small style="color: red; font-weight: bold;">${message}</small></h5>
                    ...
                    <div class="ibox-content">
                        ...
                        <table class="table table-striped table-bordered table-hover dataTables-example">
                            ...
                            <td>
                            <a class="btn btn-small btn-primary" href="<%=request.getServletContext().getContextPath()%>/admin/goods/getGoods/${v.id}" >修改</a>
                            <a class="btn btn-small btn-warning" href="" onclick="toDelete(this)" uri="<%=request.getServletContext().getContextPath()%>/admin/goods/deleteGoods/${v.id}">删除</a>
                            <c:if test="${v.status == 0 }">
                            	<a class="btn btn-small btn-info" href="<%=request.getServletContext().getContextPath()%>/admin/goods/goodsUp/${v.id}" uri="">上架</a>
                            </c:if>
                            <c:if test="${v.status == 1 }">
                            	<a class="btn btn-small btn-error" href="<%=request.getServletContext().getContextPath()%>/admin/goods/goodsdown/${v.id}">下架</a>
                            </c:if>
                            <a class="btn btn-small btn-warning" href="<%=request.getServletContext().getContextPath()%>/admin/goods/getImg/${v.id}">缩略图</a>
                            </td>
                            </tr>
                            </c:forEach>
                            </tbody>
